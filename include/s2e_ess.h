...
struct s2e_conf {
	...
	int net_mode;					/* Defines Server or Clinet Connection */
	int net_proto;					/* Defines TCP or UDP Protocol < NET_MODE_TCP:tcp connection, NET_MODE_UDP:udp connection >*/
	int net_port;
	char net_remote_ip[32];
	int tty_fd;
	int sock_fd;
	char *tty_buffer;
	int tty_buffsz;
